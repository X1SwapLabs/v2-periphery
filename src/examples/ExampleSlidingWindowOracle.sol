...
contract ExampleSlidingWindowOracle {
    ...
    function update(address tokenA, address tokenB) external {
        ...
        if (timeElapsed > periodSize) {
            (uint256 price0Cumulative, uint256 price1Cumulative,) = X1swapV2OracleLibrary.currentCumulativePrices(pair);
    ...
    function consult(address tokenIn, uint256 amountIn, address tokenOut) external view returns (uint256 amountOut) {
        address pair = X1swapV2Library.pairFor(factory, tokenIn, tokenOut);
        ...
        (uint256 price0Cumulative, uint256 price1Cumulative,) = X1swapV2OracleLibrary.currentCumulativePrices(pair);
        (address token0,) = X1swapV2Library.sortTokens(tokenIn, tokenOut);
