...
import "@x1swap/lib/src/FullMath.sol";

import "./SafeMath.sol";
import "./X1swapV2Library.sol";

// library containing some math for dealing with the liquidity shares of a pair, e.g. computing their exact value
// in terms of the underlying tokens
library X1swapV2LiquidityMathLibrary {
    ...
    function getReservesAfterArbitrage(
        ...
        (reserveA, reserveB) = X1swapV2Library.getReserves(factory, tokenA, tokenB);

        require(reserveA > 0 && reserveB > 0, "X1swapV2ArbitrageLibrary: ZERO_PAIR_RESERVES");
        ...
        if (aToB) {
            uint256 amountOut = X1swapV2Library.getAmountOut(amountIn, reserveA, reserveB);
            reserveA += amountIn;
            reserveB -= amountOut;
        } else {
            uint256 amountOut = X1swapV2Library.getAmountOut(amountIn, reserveB, reserveA);
            reserveB += amountIn;
            reserveA -= amountOut;
        }
    }
    ...
    {
        (uint256 reservesA, uint256 reservesB) = X1swapV2Library.getReserves(factory, tokenA, tokenB);
        IX1SwapV2Pair pair = IX1SwapV2Pair(X1swapV2Library.pairFor(factory, tokenA, tokenB));
        bool feeOn = IX1SwapV2Factory(factory).feeTo() != address(0);
